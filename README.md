<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:1e3c72,100:2a5298&height=180&section=header&text=Hi%20There!%20👋&fontSize=40&fontColor=ffffff" />
</p>

<h1 align="center">I'm Prakash</h1>
<h3 align="center">Aspiring AI & Machine Learning Engineer</h3>

<p align="center">
  <a href="https://github.com/your-prakash-k66">
    <img src="https://img.shields.io/github/followers/prakash-k66?label=Followers&style=flat-square" />
  </a>
  <a href="https://github.com/prakash-k66">
    <img src="https://img.shields.io/github/stars/prakash-k66?label=Stars&style=flat-square" />
  </a>
</p>

---

## 🚀 About Me

I am an **engineering student (2nd year)** with a strong interest in **Artificial Intelligence, Machine Learning, and Software Development**.  
I enjoy building **real-world, problem-solving applications** and learning by doing.

- 🎓 Engineering student  
- 🤖 Interested in **AI, ML, and LLM-based applications**
- 🧠 Learning **DSA, System Design, and Backend Development**
- 🌍 Career goal: **Work abroad (USA / Germany / UK)**
- 🔍 Passionate about turning ideas into working products

---

## 🧠 Core Interests

- Artificial Intelligence & Machine Learning  
- Large Language Models (LLMs)  
- Backend Development  
- Full-Stack Web Applications  
- Data Structures & Algorithms  
- Real-World Project Building  

---

## 🛠️ Tech Stack

### 👨‍💻 Programming Languages
- Python  
- C  
- Java (Basic)

### 🤖 AI / ML
- Machine Learning Fundamentals  
- CNNs (Image Classification)  
- LLM Integration  
- Model Deployment (Basics)

### 🌐 Web Development
- Flask  
- FastAPI  
- HTML, CSS  
- REST APIs

### 🧰 Tools & Platforms
- Git & GitHub  
- VS Code  
- JSON-based Databases  
- Linux (Basics)

---

## 📌 Featured Projects

### 🔹 AI-Powered Tutor for Visually Impaired
- Generates lessons from PDFs using LLMs  
- Text-to-Speech & Screen Display support  
- Flask-based web interface with voice interaction  

### 🔹 CookCraft AI (Smart Recipe Generator)
- Recipe generation based on available ingredients  
- Designed for students & working professionals  
- Focus on usability and real-world needs  

### 🔹 AI Chatbot using Ollama
- Integrated Gemma 2B model  
- Built with Flask  
- Focus on conversational AI and backend logic  

---

## 📈 What I’m Currently Learning

- Data Structures & Algorithms (from scratch)  
- Mathematics for Machine Learning  
- Advanced Python & OOPs  
- System Design Basics  
- Deployment & GitHub best practices  

---

## 🤝 Connect With Me

- 💻 GitHub: https://github.com/prakash-k66
- 📧 Email: your-prakashkmp12@gmail.com
- 🔗 LinkedIn:https://www.linkedin.com/in/prakash-k-239846283/

---

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=your-username&show_icons=true&theme=tokyonight" />
</p>

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=your-username&theme=tokyonight" />
</p>

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:2a5298,100:1e3c72&height=120&section=footer" />
</p>
